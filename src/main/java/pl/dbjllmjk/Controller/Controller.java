...
/**
 * Main Controller - responsible for launching login module and changing level
 * of happiness/hunger/health for {@link Pet}s.
 */
public class Controller {
    ...
    /**
     * Controls {@link Pet}'s parameters. In other words, tutorial: how to piss
     * of gamers - your only customers.
     */
    public void makeLogicUpdate() {
        ...
        this.dataRepository.getUsers().stream().forEach((user) -> {
            user.getPets().stream().forEach((pet) -> {
                ...
                if (calculateDifferenceBetweenDates(pet.getLastFeedingDate(), LocalDateTime.now()) > 60) {
                    int points = (int) (calculateDifferenceBetweenDates(pet.getLastFeedingDate(), LocalDateTime.now()) / 60);
                ...
                if (calculateDifferenceBetweenDates(pet.getLastActivityDate(), LocalDateTime.now()) > 30) {
                    int points = (int) (calculateDifferenceBetweenDates(pet.getLastActivityDate(), LocalDateTime.now()) / 30);
                ...
                if (calculateDifferenceBetweenDates(pet.getLastOperationDate(), LocalDateTime.now()) > 240) {
                    int points = (int) (calculateDifferenceBetweenDates(pet.getLastOperationDate(), LocalDateTime.now()) / 240);
                    if (points > 10) {
                        points = 10;
                    }
